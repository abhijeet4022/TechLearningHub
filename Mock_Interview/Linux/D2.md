...
- For servers behind the load balancer, we patch one server at a time. Once the patches are applied and the first server is up and running properly, we proceed to the next server deregister the server form TG during the patching.
...
4. **Take Action to Free Up Space**
    ...
    * Compress large files using `gzip` or `tar` if we can.
