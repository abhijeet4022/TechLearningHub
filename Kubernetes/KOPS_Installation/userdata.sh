...
# Update system
echo -e "\e[32mUpdating system packages...\e[0m" | tee -a ${LOG_FILE}
sudo apt-get update -y &>> ${LOG_FILE}

# Download and install kops
echo -e "\e[32mInstalling kops...\e[0m" | tee -a ${LOG_FILE}
...
echo -e "\e[32mInstalling kubectl...\e[0m" | tee -a ${LOG_FILE}
...
echo -e "\e[32mInstalling kubens...\e[0m" | tee -a ${LOG_FILE}
...
echo -e "\e[32mConfiguring environment variables and alias...\e[0m" | tee -a ${LOG_FILE}
...
echo -e "\e[32mGenerating SSH keys...\e[0m" | tee -a ${LOG_FILE}
if [ ! -f ~/.ssh/id_rsa ]; then
  ssh-keygen -t rsa -b 2048 -f ~/.ssh/id_rsa -N "" &>> ${LOG_FILE}
fi

# Create a directory for cluster deployment
echo -e "\e[32mCreating cluster deployment directory at $DEPLOYMENT_DIR...\e[0m" | tee -a ${LOG_FILE}
sudo mkdir -p ${DEPLOYMENT_DIR} &>> ${LOG_FILE}

# Generate cluster.yaml for deployment
echo -e "\e[32mGenerating cluster.yaml configuration...\e[0m" | tee -a ${LOG_FILE}
...
echo -e "\e[32mCreating the cluster...\e[0m" | tee -a ${LOG_FILE}
sudo -E kops create -f ${DEPLOYMENT_DIR}/cluster.yaml &>> ${LOG_FILE}

# Update the cluster
echo -e "\e[32mUpdating the cluster...\e[0m" | tee -a ${LOG_FILE}
sudo -E kops update cluster --name=$CLUSTER_NAME --yes --admin &>> ${LOG_FILE}
echo -e "\e[32mPlease wait for 10 Min to up the cluster...\e[0m" | tee -a ${LOG_FILE}
sleep 300

# Create the .kube directory for the root user
echo -e "\e[32mCreate the .kube directory for the root user...\e[0m" | tee -a ${LOG_FILE}
sudo mkdir -p /root/.kube  /home/ubuntu/.kube

# Move the Kubernetes configuration file to the root user's .kube directory
echo -e "\e[32mMove the Kubernetes configuration file to the root and ubuntu user's .kube directory\e[0m" | tee -a ${LOG_FILE}
sudo cp /.kube/config /home/ubuntu/.kube/config
sudo cp /.kube/config /root/.kube/config
echo -e "\e[32mSetting the .kube directory ownership and permission...\e[0m" | tee -a ${LOG_FILE}
sudo chown -R root:ubuntu /home/ubuntu/.kube
sudo chmod -R 770 /root/.kube /home/ubuntu/.kube

sleep 300
# Output a completion message
echo -e "\e[32mKubernetes configuration for root user has been set up successfully.\e[0m" | tee -a "$LOG_FILE"


## Optional: Validate the cluster
#echo -e "\e[32mValidating the cluster (optional)...\e[0m" | tee -a ${LOG_FILE}
